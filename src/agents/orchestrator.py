...
def advise(query: str, provider: str, requirements_path: str | None = None, python_version: str = "3.11", out_dir: str = "data", history: list | None = None, prompt_style: str = "compact_json", planning_mode: str = "single", verify: bool = False) -> dict:
    ...
    t2 = time.time()
    answer_text = llm_client.summarize_answer(provider, query, results, history, style=prompt_style)
    verify_ms = 0
    if verify:
        tv0 = time.time()
        answer_text = llm_client.verify_answer(provider, query, results, answer_text, style=prompt_style)
        tv1 = time.time()
